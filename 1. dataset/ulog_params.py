...
def main():
    ...
    convert_ulog_params(args.filename, output_file=args.output_filename, timestamps=args.timestamps, delimiter=args.delimiter, initial=args.initial, format=args.format, default=args.default, disable_str_exceptions=args.ignore)


####################################
# 
def convert_ulog_params(ulog_file_name, output_file, timestamps=False, delimiter=",", initial=False, format="csv", default=None, disable_str_exceptions=False):
    message_filter = []
    if not initial: message_filter = None
    ...
    # To test if the ulog file has useful data
    #if not ulog.changed_parameters == []:
    #    print("AAAAAAAAAAAAAAAAAAAA")

    params = ulog.initial_parameters
    if default is not None:
        params = get_defaults(ulog, default)
        initial = True
